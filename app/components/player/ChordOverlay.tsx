  ...
  return (
    <div className={cn('space-y-1 font-mono', className)}>
      {parsed.lines.map((line, index) => (
        <ChordOverlayLine key={index} line={line} transpose={transpose} elementId={`element-${index}`} />
      ))}
    </div>
  )
}

function ChordOverlayLine({ line, transpose, elementId }: { line: AnyParsedLine; transpose: number; elementId: string }) {
  if (line.type === 'empty') {
